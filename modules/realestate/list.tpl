<div class="ia-card ia-item--{$listing.status} {if $listing.featured}-featured{/if} {if $listing.sponsored}-sponsored{/if}">
    ...
    {if !empty($listing.gallery)}
        <a class="ia-card__image" href="{$listing.link}">
            {ia_image file=$listing.gallery[0] type='thumbnail' title=$listing.address}
        ...
        {if $listing.sponsored}
            <div class="ia-card__image__more hidden-xs">
                {foreach $listing.gallery as $pic}
                    {if !$pic@first}
                        <a href="{$listing.link}">
                            {ia_image file=$pic type='thumbnail' title=$listing.address}
                        </a>
                    {/if}
                    {if $pic@iteration == 4}
                        {break}
    ...
    <div class="ia-card__content">
        ...
        <a class="ia-card__title" href="{$listing.link}">{$listing.address} <small class="text-fade-50">{lang key='id'}: {$listing.id}</small></a>
        ...
        <div class="ia-item__additional">
            ...
            {if $listing.bedrooms}<p title="{lang key='field_estates_bedrooms'}"><span class="fa fa-bed"></span> {lang key="field_estates_bedrooms+{$listing.bedrooms}"}</p>{/if}
            {if $listing.bathrooms}<p title="{lang key='field_estates_bathrooms'}"><span class="fa fa-tint"></span> {lang key="field_estates_bathrooms+{$listing.bathrooms}"}</p>{/if}
